...
export function trendingClasses(value: number) {
    ...
    return {
        textClass: isNeutral ? 'text-gray-400' : isTrendingUp ? 'text-green-400' : 'text-red-400',
        bgClass: isNeutral ? 'bg-gray-500/10' : isTrendingUp ? 'bg-green-500/10' : 'bg-red-500/10',
        iconClass: isNeutral ? 'icon-neutral' : isTrendingUp ? 'icon-up' : 'icon-down',
    };
}
